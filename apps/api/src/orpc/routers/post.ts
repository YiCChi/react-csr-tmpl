...
export const postRouter = {
  // Query: 获取单个文章
  getPost: publicProcedure.input(z.object({ id: z.number() })).handler(({ input }) => {
  ...
  uploadSingleFile: publicProcedure.input(z.instanceof(File)).handler(async ({ input: file }) => {
    ...
    const timestamp = Date.now();
    const fileExtension = path.extname(file.name);
    const uniqueFileName = `post_${postId}_${timestamp}${fileExtension}`;
    const filePath = path.join(uploadDir, uniqueFileName);
  ...
  uploadFileWithPost: publicProcedure
    .input(
      z.object({
        postId: z.number(),
  ...
  // Query: 获取热门标签
  getPopularTags: publicProcedure.handler(() => {
